...
create table if not exists profiles (
  ...
  yakker_id text unique not null,
  karma int default 0,
...
create table if not exists communities (
  ...
  radius_meters int default 5000 check (radius_meters <= 15000),
  created_at timestamp with time zone default timezone('utc'::text, now()) not null
);
...
create table if not exists ads (
  id uuid default gen_random_uuid() primary key,
  business_owner_id uuid references auth.users not null,
  title text,
  content text not null,
...
drop policy if exists "Users can update own communities" on communities;
...
create policy "Users can update own communities" on communities for update using (auth.uid() = creator_id);
create policy "Users can delete own communities" on communities for delete using (auth.uid() = creator_id);
...
create policy "Users can delete own ads" on ads for delete using (auth.uid() = business_owner_id);
